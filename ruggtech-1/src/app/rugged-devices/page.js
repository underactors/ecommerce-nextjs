...
export default function RuggedDevicesPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch('/api/products?category=rugged-devices');
    ...
    return (
      <div className="container" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-gray)', fontSize: '18px' }}>Loading rugged devices...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container">
      <nav style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '20px 0', fontSize: '14px', color: 'var(--text-gray)' }}>
        ...
        <span>/</span>
        <span style={{ color: 'var(--text-light)' }}>Rugged Devices</span>
      </nav>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '12px', background: 'rgba(59, 130, 246, 0.3)', padding: '8px 20px', borderRadius: '30px', marginBottom: '16px' }}>
            <FontAwesomeIcon icon={faShieldAlt} style={{ color: '#60a5fa' }} />
            <span style={{ color: 'white', fontWeight: '600', fontSize: '14px' }}>BUILT TOUGH</span>
          </div>
          
          <h1 style={{ color: 'white', fontSize: 'clamp(28px, 5vw, 42px)', fontWeight: '800', marginBottom: '12px', textShadow: '0 2px 10px rgba(0,0,0,0.3)' }}>
            <FontAwesomeIcon icon={faMobileAlt} style={{ marginRight: '12px' }} />
            Rugged Devices
          </h1>
          
          <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: '18px', maxWidth: '600px', margin: '0 auto' }}>
            Durable, waterproof, and shockproof devices built for extreme conditions. Perfect for outdoor adventures and industrial use.
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', marginBottom: '30px', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '12px', padding: '20px 30px', display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ background: 'linear-gradient(135deg, #3b82f6, #1d4ed8)', width: '50px', height: '50px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {filteredProducts.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '80px 20px', background: 'var(--card-bg)', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
          <FontAwesomeIcon icon={faShieldAlt} size="4x" style={{ color: 'var(--text-gray)', marginBottom: '20px' }} />
          <h2 style={{ color: 'var(--text-light)', marginBottom: '10px' }}>No Products Available</h2>
          <p style={{ color: 'var(--text-gray)', marginBottom: '30px' }}>Check back soon for new rugged devices!</p>
